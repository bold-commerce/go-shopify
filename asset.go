...
func (s *AssetServiceOp) List(themeID int64, options interface{}) ([]Asset, error) {
	path := fmt.Sprintf("%s/%s/%d/assets.json", GetAdminVersionedApiPathPrefix(), assetsBasePath, themeID)
...
func (s *AssetServiceOp) Delete(themeID int64, key string) error {
	path := fmt.Sprintf("%s/%s/%d/assets.json?asset[key]=%s", GetAdminVersionedApiPathPrefix(), assetsBasePath, themeID, key)
