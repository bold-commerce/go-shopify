...
func (s *BlogServiceOp) List(options interface{}) ([]Blog, error) {
	path := fmt.Sprintf("%s/%s.json", GetAdminVersionedApiPathPrefix(), blogsBasePath)
...
func (s *BlogServiceOp) Count(options interface{}) (int, error) {
	path := fmt.Sprintf("%s/%s/count.json", GetAdminVersionedApiPathPrefix(), blogsBasePath)
	return s.client.Count(path, options)
}

// Get single blog
func (s *BlogServiceOp) Get(blogId int64, options interface{}) (*Blog, error) {
	path := fmt.Sprintf("%s/%s/%d.json", GetAdminVersionedApiPathPrefix(), blogsBasePath, blogId)
...
func (s *BlogServiceOp) Update(blog Blog) (*Blog, error) {
	path := fmt.Sprintf("%s/%s/%d.json", GetAdminVersionedApiPathPrefix(), blogsBasePath, blog.ID)
	wrappedData := BlogResource{Blog: &blog}
	resource := new(BlogResource)
...
func (s *BlogServiceOp) Delete(blogId int64) error {
	return s.client.Delete(fmt.Sprintf("%s/%s/%d.json", GetAdminVersionedApiPathPrefix(), blogsBasePath, blogId))
}
