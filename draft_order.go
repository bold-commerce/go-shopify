...
func (s *DraftOrderServiceOp) Create(draftOrder DraftOrder) (*DraftOrder, error) {
	path := fmt.Sprintf("%s/%s.json", GetAdminVersionedApiPathPrefix(), draftOrdersBasePath)
...
func (s *DraftOrderServiceOp) Count(options interface{}) (int, error) {
	path := fmt.Sprintf("%s/%s/count.json", GetAdminVersionedApiPathPrefix(), draftOrdersBasePath)
	return s.client.Count(path, options)
}

// Delete draft orders
func (s *DraftOrderServiceOp) Delete(draftOrderID int64) error {
	path := fmt.Sprintf("%s/%s/%d.json", GetAdminVersionedApiPathPrefix(), draftOrdersBasePath, draftOrderID)
...
func (s *DraftOrderServiceOp) Invoice(draftOrderID int64, draftOrderInvoice DraftOrderInvoice) (*DraftOrderInvoice, error) {
	path := fmt.Sprintf("%s/%s/%d/send_invoice.json", GetAdminVersionedApiPathPrefix(), draftOrdersBasePath, draftOrderID)
...
func (s *DraftOrderServiceOp) Update(draftOrder DraftOrder) (*DraftOrder, error) {
	path := fmt.Sprintf("%s/%s/%d.json", GetAdminVersionedApiPathPrefix(), draftOrdersBasePath, draftOrder.ID)
	wrappedData := DraftOrderResource{DraftOrder: &draftOrder}
	resource := new(DraftOrderResource)
...
func (s *DraftOrderServiceOp) Complete(draftOrderID int64, paymentPending bool) (*DraftOrder, error) {
	path := fmt.Sprintf("%s/%s/%d/complete.json?payment_pending=%t", GetAdminVersionedApiPathPrefix(), draftOrdersBasePath, draftOrderID, paymentPending)
	resource := new(DraftOrderResource)
