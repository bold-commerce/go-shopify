...
func (s *WebhookServiceOp) List(options interface{}) ([]Webhook, error) {
	path := fmt.Sprintf("%s/%s.json", GetAdminVersionedApiPathPrefix(), webhooksBasePath)
...
func (s *WebhookServiceOp) Count(options interface{}) (int, error) {
	path := fmt.Sprintf("%s/%s/count.json", GetAdminVersionedApiPathPrefix(), webhooksBasePath)
	return s.client.Count(path, options)
}

// Get individual webhook
func (s *WebhookServiceOp) Get(webhookdID int64, options interface{}) (*Webhook, error) {
	path := fmt.Sprintf("%s/%s/%d.json", GetAdminVersionedApiPathPrefix(), webhooksBasePath, webhookdID)
...
func (s *WebhookServiceOp) Update(webhook Webhook) (*Webhook, error) {
	path := fmt.Sprintf("%s/%s/%d.json", GetAdminVersionedApiPathPrefix(), webhooksBasePath, webhook.ID)
	wrappedData := WebhookResource{Webhook: &webhook}
	resource := new(WebhookResource)
...
func (s *WebhookServiceOp) Delete(ID int64) error {
	return s.client.Delete(fmt.Sprintf("%s/%s/%d.json", GetAdminVersionedApiPathPrefix(), webhooksBasePath, ID))
}
