...
func (s *ImageServiceOp) List(productID int64, options interface{}) ([]Image, error) {
	path := fmt.Sprintf("%s/%s/%d/images.json", GetAdminVersionedApiPathPrefix(), productsBasePath, productID)
...
func (s *ImageServiceOp) Count(productID int64, options interface{}) (int, error) {
	path := fmt.Sprintf("%s/%s/%d/images/count.json", GetAdminVersionedApiPathPrefix(), productsBasePath, productID)
	return s.client.Count(path, options)
}

// Get individual image
func (s *ImageServiceOp) Get(productID int64, imageID int64, options interface{}) (*Image, error) {
	path := fmt.Sprintf("%s/%s/%d/images/%d.json", GetAdminVersionedApiPathPrefix(), productsBasePath, productID, imageID)
...
func (s *ImageServiceOp) Update(productID int64, image Image) (*Image, error) {
	path := fmt.Sprintf("%s/%s/%d/images/%d.json", GetAdminVersionedApiPathPrefix(), productsBasePath, productID, image.ID)
	wrappedData := ImageResource{Image: &image}
	resource := new(ImageResource)
...
func (s *ImageServiceOp) Delete(productID int64, imageID int64) error {
	return s.client.Delete(fmt.Sprintf("%s/%s/%d/images/%d.json", GetAdminVersionedApiPathPrefix(), productsBasePath, productID, imageID))
}
