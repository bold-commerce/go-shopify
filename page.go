...
func (s *PageServiceOp) List(options interface{}) ([]Page, error) {
	path := fmt.Sprintf("%s/%s.json", GetAdminVersionedApiPathPrefix(), pagesBasePath)
...
func (s *PageServiceOp) Count(options interface{}) (int, error) {
	path := fmt.Sprintf("%s/%s/count.json", GetAdminVersionedApiPathPrefix(), pagesBasePath)
	return s.client.Count(path, options)
}

// Get individual page
func (s *PageServiceOp) Get(pageID int64, options interface{}) (*Page, error) {
	path := fmt.Sprintf("%s/%s/%d.json", GetAdminVersionedApiPathPrefix(), pagesBasePath, pageID)
...
func (s *PageServiceOp) Update(page Page) (*Page, error) {
	path := fmt.Sprintf("%s/%s/%d.json", GetAdminVersionedApiPathPrefix(), pagesBasePath, page.ID)
	wrappedData := PageResource{Page: &page}
	resource := new(PageResource)
...
func (s *PageServiceOp) Delete(pageID int64) error {
	return s.client.Delete(fmt.Sprintf("%s/%s/%d.json", GetAdminVersionedApiPathPrefix(), pagesBasePath, pageID))
}
