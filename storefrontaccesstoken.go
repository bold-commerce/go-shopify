...
func (s *StorefrontAccessTokenServiceOp) List(options interface{}) ([]StorefrontAccessToken, error) {
	path := fmt.Sprintf("%s/%s.json", globalPathVersionPrefix, storefrontAccessTokensBasePath)
...
func (s *StorefrontAccessTokenServiceOp) Delete(ID int64) error {
	return s.client.Delete(fmt.Sprintf("%s/%s/%d.json", globalPathVersionPrefix, storefrontAccessTokensBasePath, ID))
}
