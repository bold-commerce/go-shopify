...
func (s *TransactionServiceOp) List(orderID int64, options interface{}) ([]Transaction, error) {
	path := fmt.Sprintf("%s/%s/%d/transactions.json", GetAdminVersionedApiPathPrefix(), ordersBasePath, orderID)
...
func (s *TransactionServiceOp) Count(orderID int64, options interface{}) (int, error) {
	path := fmt.Sprintf("%s/%s/%d/transactions/count.json", GetAdminVersionedApiPathPrefix(), ordersBasePath, orderID)
...
func (s *TransactionServiceOp) Get(orderID int64, transactionID int64, options interface{}) (*Transaction, error) {
	path := fmt.Sprintf("%s/%s/%d/transactions/%d.json", GetAdminVersionedApiPathPrefix(), ordersBasePath, orderID, transactionID)
