...
func (s *RedirectServiceOp) List(options interface{}) ([]Redirect, error) {
	path := fmt.Sprintf("%s/%s.json", GetAdminVersionedApiPathPrefix(), redirectsBasePath)
...
func (s *RedirectServiceOp) Count(options interface{}) (int, error) {
	path := fmt.Sprintf("%s/%s/count.json", GetAdminVersionedApiPathPrefix(), redirectsBasePath)
	return s.client.Count(path, options)
}

// Get individual redirect
func (s *RedirectServiceOp) Get(redirectID int64, options interface{}) (*Redirect, error) {
	path := fmt.Sprintf("%s/%s/%d.json", GetAdminVersionedApiPathPrefix(), redirectsBasePath, redirectID)
...
func (s *RedirectServiceOp) Update(redirect Redirect) (*Redirect, error) {
	path := fmt.Sprintf("%s/%s/%d.json", GetAdminVersionedApiPathPrefix(), redirectsBasePath, redirect.ID)
	wrappedData := RedirectResource{Redirect: &redirect}
	resource := new(RedirectResource)
...
func (s *RedirectServiceOp) Delete(redirectID int64) error {
	return s.client.Delete(fmt.Sprintf("%s/%s/%d.json", GetAdminVersionedApiPathPrefix(), redirectsBasePath, redirectID))
}
